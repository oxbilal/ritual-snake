...
function compile() {
  const sourcePath = "contracts/RitualSnakeScores.sol";
  const input = {
    language: "Solidity",
    sources: {
  ...
  const contract = output.contracts[sourcePath].RitualSnakeScores;
  return {
    abi: contract.abi,
...
if (compileOnly) {
  console.log("RitualSnakeScores compiles successfully.");
  process.exit(0);
}
...
if (receipt.status !== "success" || !receipt.contractAddress) {
  throw new Error(`RitualSnakeScores deployment failed: ${hash}`);
}

const existing = fs.existsSync(envPath) ? fs.readFileSync(envPath, "utf8") : "";
const lines = existing
  .split(/\r?\n/)
  .filter((line) => line.trim() && !line.startsWith("NEXT_PUBLIC_RITUAL_SNAKE_SCORES="));
lines.push(`NEXT_PUBLIC_RITUAL_SNAKE_SCORES=${receipt.contractAddress}`);
fs.writeFileSync(envPath, `${lines.join("\n")}\n`);

console.log(`Deployed contract address: ${receipt.contractAddress}`);
console.log(`NEXT_PUBLIC_RITUAL_SNAKE_SCORES=${receipt.contractAddress}`);
